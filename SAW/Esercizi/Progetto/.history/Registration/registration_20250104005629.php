...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["submit"])) {
    ...
    if (empty($errors)) {
        ...
        file_put_contents("Es.json", $json);
        if (file_exists("Es.json")) {
            $existingData = json_decode(file_get_contents("Es.json"), true);
            if (is_array($existingData)) {
                $existingData[] = $data;
            } else {
                $existingData = [$data];
            }
            file_put_contents("Es.json", json_encode($existingData));
        } else {
            file_put_contents("Es.json", json_encode([$data]));
        }
    }
