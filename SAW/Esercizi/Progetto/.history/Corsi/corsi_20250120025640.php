...
if (isset($_GET['get_course']) || isset($_GET['search'])) {
    ...
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $searchTerm = htmlspecialchars($searchTerm, ENT_QUOTES, 'UTF-8'); //sfrutto specialchars e non htmlentities perchè htmlentities converte anche lettere accentate
        ...
        $stmt = $conn->prepare("SELECT * FROM Corsi WHERE nome LIKE ? OR categoria LIKE ?");
    ...
    // Ottiengp gli id dei corsi per cui l'utente è già registrato
    $userCourses = [];
    if ($email) {
