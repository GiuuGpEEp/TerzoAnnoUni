        ...
        .then(data => {
            ...
            data.forEach(corso => {
                ...
                container.innerHTML = `
                    ...
                    <button class="${buttonClass}" id="prenotationButton">Prenotati</button> `;
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const loginStatus = "<?php echo $loginStatus; ?>";  // Stato di login dell'utente

        // Seleziona tutti i bottoni con la classe dinamica assegnata
        const buttons = document.querySelectorAll('.prenotationButtonLogin, .prenotationButtonNoLogin');

        // Aggiungi un listener di evento per ogni bottone
        buttons.forEach(function(button) {
            button.addEventListener('click', function() {
                // Se l'utente non è loggato, mostra un alert
                if (loginStatus == 'false') {
                    window.alert('Devi effettuare il login per prenotare un corso');
                }
            });
        });
    });
