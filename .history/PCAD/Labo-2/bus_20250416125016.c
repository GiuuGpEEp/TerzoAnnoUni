...
void* bus_function() {
    ...
    // Fai salire C passeggeri
    for (int i = 0; i < C; i++) {
        my_sem_signal(&peopleSemaphore);
    }

    // Aspetta che tutti e soli C passeggeri siano saliti
    for (int i = 0; i < C; i++) {
        my_sem_wait(&busSemaphore);
    }

    printf("Tutti i passeggeri sono saliti, pronti a partire...\n");
    sleep(2); // Simula il viaggio
...
void* person_function(void* arg) {
    int id = *(int*)arg;
    free(arg);  // Importante per evitare memory leak

    my_sem_wait(&peopleSemaphore); // Aspetta di poter salire

    pthread_mutex_lock(&countMutex);
    people_count++;
    printf("👤 Persona %d: salita. A bordo: %d\n", id, people_count);
    pthread_mutex_unlock(&countMutex);

    my_sem_signal(&busSemaphore); // Segnala al bus che uno è salito
...
int main() {
    ...
    my_sem_init(&busSemaphore, 0);        // Inizialmente 0, il bus aspetta che le persone segnalino
    my_sem_init(&peopleSemaphore, 0);     // Inizialmente chiuso, il bus lo apre per i primi C
