...
document.addEventListener("DOMContentLoaded", function() { //inserendo questo ho la certezza che prima si eseguire il javascript sia caricato tutto il DOM
    ...
    calc.addEventListener("click", () => {
        ...
        clessidra.classList.toggle("rotate"); //Il metodo toggle agisce sull'elenco delle classi (classList) di un elemento HTML. Funziona come un interruttore: se la classe specificata è già presente nell'elemento, toggle la rimuove; se non è presente, toggle la aggiunge.
    });
    ...
    closeButton.addEventListener("click",() =>{
        let tmp = clessidra;
        clessidra.style.transition = "none";
        document.getElementById("targetIframe").src = "about:blank";
        document.getElementById("targetIframe").style.border = "none";
        testoCless.classList.remove("fadeInAnimation");
        clessidra.style.transition = tmp.style.transition;
    })
