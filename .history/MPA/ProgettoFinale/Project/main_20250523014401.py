...
import pandas as pd
from Models import TrainerDefinitivo as trainer

def run_and_get_metrics():
    models = {
        "10min": trainer.train_and_evaluate(minuti=10, verbose=False),
        "15min": trainer.train_and_evaluate(minuti=15, verbose=False),
        "20min": trainer.train_and_evaluate(minuti=20, verbose=False),
        "25min": trainer.train_and_evaluate(minuti=25, verbose=False),
        "30min": trainer.train_and_evaluate(minuti=30, verbose=False),
    }
...
def run_single_model():
    ...
    results = trainer.train_and_evaluate(minuti=minuti, verbose=True)
    print("\n[METRICHE]")
