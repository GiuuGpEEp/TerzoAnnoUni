...
def parse_tower_list_detailed(tower_raw, prefix, minuti=10):
    stats = {f"{prefix}_{key}": 0 for key in [
        f"towers_{minuti}min", "top_towers", "mid_towers", "bot_towers",
    ...
    for tower in tower_raw:
        ...
        if isinstance(tower, (list, tuple)) and len(tower) >= 3:
            try:
                ...
                if minute < minuti:
                    stats[f"{prefix}_towers_{minuti}min"] += 1
...
def parse_event_list_simple(event_raw, prefix, event_name, minuti=10):
    count = 0
    ...
    if isinstance(event_raw, list):
        for minute in event_raw:
            try:
                if float(minute) < minuti:
                    count += 1
            except:
                continue
    return {f"{prefix}_{event_name}_{minuti}min": count}

def extract_features(row, minuti=10):
    features = {}
    ...
    if isinstance(gold_diff, list) and gold_diff:
        first_n = gold_diff[:minuti]
        features[f"gold_diff_slope_0_{minuti}"] = calc_slope(first_n)
        features[f"gold_diff_slope_0_{minuti//2}"] = calc_slope(first_n[:minuti//2])
        features[f"gold_diff_slope_{minuti//2}_{minuti}"] = calc_slope(first_n[minuti//2:minuti])
        features[f"avg_gold_diff_{minuti}min"] = np.mean(first_n)
        features[f"gold_diff_min{minuti}"] = first_n[minuti-1] if len(first_n) >= minuti else first_n[-1]
        features[f"gold_diff_variance_{minuti}min"] = np.var(first_n)
        features[f"gold_diff_delta_last{minuti//2}min"] = first_n[minuti-1] - first_n[minuti//2-1] if len(first_n) >= minuti else 0
    else:
        for key in [f"gold_diff_slope_0_{minuti}", f"gold_diff_slope_0_{minuti//2}", f"gold_diff_slope_{minuti//2}_{minuti}",
                    f"avg_gold_diff_{minuti}min", f"gold_diff_min{minuti}", f"gold_diff_variance_{minuti}min",
                    f"gold_diff_delta_last{minuti//2}min"]:
            features[key] = 0
    ...
    features.update(parse_event_list_simple(b_kills, "b", "kills", minuti))
    features.update(parse_event_list_simple(r_kills, "r", "kills", minuti))
    features.update(parse_event_list_simple(b_dragons, "b", "dragons", minuti))
    features.update(parse_event_list_simple(r_dragons, "r", "dragons", minuti))
    ...
    for team in ['b', 'r']:
        for event in ['kills', 'dragons']:
            features[f"{team}_{event}_0_{minuti//2}"] = count_events_in_range(row.get(f"{team.capitalize()}{event.capitalize()}", []), 0, minuti//2)
            features[f"{team}_{event}_{minuti//2}_{minuti}"] = count_events_in_range(row.get(f"{team.capitalize()}{event.capitalize()}", []), minuti//2, minuti)
    ...
    features[f"tower_diff_{minuti}min"] = b_tower_stats[f"b_towers_{minuti}min"] - r_tower_stats[f"r_towers_{minuti}min"]
