...
def parse_tower_list_detailed(tower_raw, prefix, max_minute):
    ...
    # Aggiunta chiave aggregata per coerenza con tower_diff
    stats[f"{prefix}_towers_{max_minute}min"] = stats[f"{prefix}_towers_limited"]
    return stats
...
def extract_features(row, minute):
    ...
    if isinstance(gold_diff, list) and gold_diff:
        ...
        features[f"gold_diff_slope_0_{mid}"] = calc_slope(first_n[:mid])
        features[f"gold_diff_slope_{mid}_{minute}"] = calc_slope(first_n[mid:minute])
        ...
        if len(first_n) >= minute:
            features[f"gold_diff_delta_last{mid}min"] = first_n[minute - 1] - first_n[mid - 1]
        else:
            features[f"gold_diff_delta_last{mid}min"] = 0
    else:
        for key in [
    ...
    # Funzione di utilità per contare eventi in un intervallo
    ...
    # Differenze kills e dragons
    features[f"kills_diff_{minute}min"] = (
        features.get(f"b_kills_{minute}min", 0) - features.get(f"r_kills_{minute}min", 0)
    )
    features[f"dragons_diff_{minute}min"] = (
        features.get(f"b_dragons_{minute}min", 0) - features.get(f"r_dragons_{minute}min", 0)
    )

    # TOWERS
    # TOWERS
    b_tower_stats = parse_tower_list_detailed(row.get("bTowers", []), "b", minute)
    r_tower_stats = parse_tower_list_detailed(row.get("rTowers", []), "r", minute)
    features.update(b_tower_stats)
    features.update(r_tower_stats)
    features[f"tower_diff_{minute}min"] = (
    b_tower_stats.get(f"b_towers_{minute}min", 0) - r_tower_stats.get(f"r_towers_{minute}min", 0)
)
