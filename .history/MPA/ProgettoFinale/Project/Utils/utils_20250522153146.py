...
def extract_features(row, minuti):
    ...
    for team in ['b', 'r']:
        for event in ['kills', 'dragons']:
            features.update(parse_event_list_simple(row.get(f"{team.upper()}{event.capitalize()}", []), team, event, minuti))
            features[f"{team}_{event}_0_{minuti//2}"] = count_events_in_range(row.get(f"{team.upper()}{event.capitalize()}", []), 0, minuti//2)
            features[f"{team}_{event}_{minuti//2}_{minuti}"] = count_events_in_range(row.get(f"{team.upper()}{event.capitalize()}", []), minuti//2, minuti)
