...
def parse_tower_list_detailed(tower_raw, prefix):
    stats = {f"{prefix}_{key}": 0 for key in [
        "towers_15min", "top_towers", "mid_towers", "bot_towers",
    ...
    for tower in tower_raw:
        ...
        if isinstance(tower, (list, tuple)) and len(tower) >= 3:
            try:
                ...
                if minute < 15:
                    stats[f"{prefix}_towers_15min"] += 1
    ...
    if isinstance(event_raw, list):
        for minute in event_raw:
            try:
                ...
                continue
    return {f"{prefix}_{event_name}_15min": count}
...
def extract_features(row):
    ...
    if isinstance(gold_diff, list) and gold_diff:
        first_15 = gold_diff[:15]
        features["gold_diff_slope_0_15"] = calc_slope(first_15)
        features["gold_diff_slope_0_5"] = calc_slope(first_15[:5])
        features["gold_diff_slope_5_10"] = calc_slope(first_15[5:10])
        features["gold_diff_slope_10_15"] = calc_slope(first_15[10:15])
        features["avg_gold_diff_15min"] = np.mean(first_15)
        features["gold_diff_min15"] = first_15[14] if len(first_15) >= 15 else first_15[-1]
        features["gold_diff_variance_15min"] = np.var(first_15)
        features["gold_diff_delta_last5min"] = first_15[14] - first_15[9] if len(first_15) >= 15 else 0
    else:
        for key in ["gold_diff_slope_0_15", "gold_diff_slope_0_5", "gold_diff_slope_5_10", "gold_diff_slope_10_15",
                    "avg_gold_diff_15min", "gold_diff_min15", "gold_diff_variance_15min",
    ...
    features["kills_diff_15min"] = features["b_kills_15min"] - features["r_kills_15min"]
    features["dragons_diff_15min"] = features["b_dragons_15min"] - features["r_dragons_15min"]
    for team in ['b', 'r']:
    ...
    features["tower_diff_15min"] = b_tower_stats["b_towers_15min"] - r_tower_stats["r_towers_15min"]
    return pd.Series(features)

# ------------------------- CARICAMENTO E MODELLAZIONE -------------------------

df = pd.read_excel('../Tables/DatiTraining15Min.xlsx')
