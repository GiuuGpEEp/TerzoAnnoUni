...
def parse_tower_list_detailed(tower_raw, prefix):
    stats = {f"{prefix}_{key}": 0 for key in [
        "towers_30min", "top_towers", "mid_towers", "bot_towers",
    ...
    for tower in tower_raw:
        ...
        if isinstance(tower, (list, tuple)) and len(tower) >= 3:
            try:
                ...
                if minute < 30:
                    stats[f"{prefix}_towers_30min"] += 1
    ...
    if isinstance(event_raw, list):
        for minute in event_raw:
            try:
                ...
                continue
    return {f"{prefix}_{event_name}_30min": count}
...
def extract_features(row):
    ...
    if isinstance(gold_diff, list) and gold_diff:
        first_30 = gold_diff[:30]
        features["gold_diff_slope_0_30"] = calc_slope(first_30)
        features["gold_diff_slope_0_5"] = calc_slope(first_30[:5])
        features["gold_diff_slope_5_10"] = calc_slope(first_30[5:10])
        features["gold_diff_slope_10_15"] = calc_slope(first_30[10:15])
        features["gold_diff_slope_15_20"] = calc_slope(first_30[15:20])
        features["gold_diff_slope_20_25"] = calc_slope(first_30[20:25])
        features["gold_diff_slope_25_30"] = calc_slope(first_30[25:30])
        features["avg_gold_diff_30min"] = np.mean(first_30)
        features["gold_diff_min30"] = first_30[29] if len(first_30) >= 30 else first_30[-1]
        features["gold_diff_variance_30min"] = np.var(first_30)
        features["gold_diff_delta_last5min"] = first_30[29] - first_30[24] if len(first_30) >= 30 else 0
    else:
        for key in ["gold_diff_slope_0_30", "gold_diff_slope_0_5", "gold_diff_slope_5_10",
                    "gold_diff_slope_10_15", "gold_diff_slope_15_20", "gold_diff_slope_20_25",
                    "gold_diff_slope_25_30", "avg_gold_diff_30min", "gold_diff_min30",
                    "gold_diff_variance_30min", "gold_diff_delta_last5min"]:
            features[key] = 0
    ...
    features["kills_diff_30min"] = features["b_kills_30min"] - features["r_kills_30min"]
    features["dragons_diff_30min"] = features["b_dragons_30min"] - features["r_dragons_30min"]
    for team in ['b', 'r']:
        for event in ['kills', 'dragons']:
            ...
            features[f"{team}_{event}_20_25"] = count_events_in_range(row.get(f"{team.capitalize()}{event.capitalize()}", []), 20, 25)
            features[f"{team}_{event}_25_30"] = count_events_in_range(row.get(f"{team.capitalize()}{event.capitalize()}", []), 25, 30)
    ...
    features["tower_diff_30min"] = b_tower_stats["b_towers_30min"] - r_tower_stats["r_towers_30min"]
    return pd.Series(features)

# ------------------------- CARICAMENTO E MODELLAZIONE -------------------------

df = pd.read_excel('../Tables/DatiTraining30Min.xlsx')
