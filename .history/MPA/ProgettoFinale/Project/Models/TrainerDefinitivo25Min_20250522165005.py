...
def parse_tower_list_detailed(tower_raw, prefix):
    stats = {f"{prefix}_{key}": 0 for key in [
        "towers_25min", "top_towers", "mid_towers", "bot_towers",
    ...
    for tower in tower_raw:
        ...
        if isinstance(tower, (list, tuple)) and len(tower) >= 3:
            try:
                ...
                if minute < 25:
                    stats[f"{prefix}_towers_25min"] += 1
    ...
    if isinstance(event_raw, list):
        for minute in event_raw:
            try:
                ...
                continue
    return {f"{prefix}_{event_name}_25min": count}
...
def extract_features(row):
    ...
    if isinstance(gold_diff, list) and gold_diff:
        first_25 = gold_diff[:25]
        features["gold_diff_slope_0_25"] = calc_slope(first_25)
        features["gold_diff_slope_0_5"] = calc_slope(first_25[:5])
        features["gold_diff_slope_5_10"] = calc_slope(first_25[5:10])
        features["gold_diff_slope_10_15"] = calc_slope(first_25[10:15])
        features["gold_diff_slope_15_20"] = calc_slope(first_25[15:20])
        features["gold_diff_slope_20_25"] = calc_slope(first_25[20:25])
        features["avg_gold_diff_25min"] = np.mean(first_25)
        features["gold_diff_min25"] = first_25[24] if len(first_25) >= 25 else first_25[-1]
        features["gold_diff_variance_25min"] = np.var(first_25)
        features["gold_diff_delta_last5min"] = first_25[24] - first_25[19] if len(first_25) >= 25 else 0
    else:
        for key in ["gold_diff_slope_0_25", "gold_diff_slope_0_5", "gold_diff_slope_5_10",
                    "gold_diff_slope_10_15", "gold_diff_slope_15_20", "gold_diff_slope_20_25",
                    "avg_gold_diff_25min", "gold_diff_min25", "gold_diff_variance_25min",
    ...
    features["kills_diff_25min"] = features["b_kills_25min"] - features["r_kills_25min"]
    features["dragons_diff_25min"] = features["b_dragons_25min"] - features["r_dragons_25min"]
    for team in ['b', 'r']:
    ...
    features["tower_diff_25min"] = b_tower_stats["b_towers_25min"] - r_tower_stats["r_towers_25min"]
    return pd.Series(features)

# ------------------------- CARICAMENTO E MODELLAZIONE -------------------------

df = pd.read_excel('../Tables/DatiTraining25Min.xlsx')
