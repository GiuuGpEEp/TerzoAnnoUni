...
def train_and_evaluate(minuti=30, verbose=False):
    minuti = 30
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    file_path = os.path.join(BASE_DIR, '..', 'Tables', 'Dati30Min.xlsx')
...
if __name__ == "__main__":
    risultati = train_and_evaluate(minuti=30, verbose=True)
    print(risultati)
    # Risultati finali
    print(f"Accuratezza: {risultati['accuracy']:.4f}")
    print(f"F1 Score: {risultati['f1']:.4f}")
    print(f"Recall: {risultati['recall']:.4f}")
    print(f"Precision: {risultati['precision']:.4f}")
    print("Modello addestrato e valutato con successo.")
