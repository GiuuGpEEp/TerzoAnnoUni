...
def extract_features_general(row, minute=10):
    features = {}
    ...
    for team in ['b', 'r']:
        for event in ['kills', 'dragons']:
            ...
            features.update(parse_event_list_simple(event_list, team, event))
    ...
    features[f"tower_diff_{minute}min"] = b_tower_stats["b_towers_10min"] - r_tower_stats["r_towers_10min"] if minute == 10 else b_tower_stats.get(f"b_towers_{minute}min", 0) - r_tower_stats.get(f"r_towers_{minute}min", 0)
