...
# Funzione per convertire le kills in liste
def parse_kill_list(kill_raw):
    if isinstance(kill_raw, str):
        try:
            kill_raw = ast.literal_eval(kill_raw)
        except:
            return []
    if not isinstance(kill_raw, list):
        return []
    return [float(k) for k in kill_raw if isinstance(k, (int, float, str)) and float(k) < 10]
...
def extract_features(row):
    ...
    # Faccio il parsing delle kill 
    b_kills = parse_kill_list(row.get("bKills", []))
    r_kills = parse_kill_list(row.get("rKills", []))
