...
# Split train/test
X_train, X_test, y_train, y_test = train_test_split(features_df, labels, test_size=0.5, random_state=42)

# Applica SMOTE solo al training set
smote = SMOTE(random_state=42)
X_train_resampled, y_train_resampled = smote.fit_resample(X_train, y_train)

# Calcola il rapporto tra le classi nel set di training
num_negatives = y_train.value_counts().get(0, 0)
num_positives = y_train.value_counts().get(1, 0)
scale_pos_weight_value = num_negatives / num_positives if num_positives > 0 else 1

# Addestra XGBoost
# Inizializza e addestra il modello XGBoost con il peso della classe
