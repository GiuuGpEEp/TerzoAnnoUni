...
def calc_slope(values):
    ...
    y = np.array(values)
    # Fit lineare semplice y = ax + b, ritorna a (coefficiente angolare)
...
def extract_features(row):
    ...
    if isinstance(gold_diff, list) and len(gold_diff) > 0:    
        ...
        # Dentro extract_features(), dopo aver estratto first_10
        features["gold_diff_slope_0_10"] = calc_slope(first_10)
        # Oppure per prime 5 min e ultimi 5 min
        if len(gold_diff) >= 10:
            features["gold_diff_slope_0_5"] = calc_slope(gold_diff[:5])
            features["gold_diff_slope_5_10"] = calc_slope(gold_diff[5:10])
        else:
            features["gold_diff_slope_0_5"] = 0
            features["gold_diff_slope_5_10"] = 0
