...
def plot_class_distribution(y_train, y_train_res): #Stampa primo grafico sulla distribuzione delle classi
...
def calc_slope(values): #Calcola la pendenza della retta di regressione lineare rappresentante la variazione del gold diff
    if len(values) < 2:
        return 0
...
def analizza_errori(X_test, y_test, y_pred, df_original): #stampa i dettagli dei falsi positivi e negativi
...
# MODIFICA: Rimuoviamo SMOTE e calcoliamo scale_pos_weight
neg, pos = np.bincount(y_train)
scale_pos_weight = neg / pos
print(f"Classe 0: {neg}, Classe 1: {pos}, scale_pos_weight: {scale_pos_weight:.2f}")

# MODIFICA: Tuning con RandomizedSearchCV
param_dist = {
...
model = RandomizedSearchCV(xgb, param_distributions=param_dist, n_iter=20, 
...
# MODIFICA: Soglia ottimale da precision-recall
y_proba = best_model.predict_proba(X_test_scaled)[:, 1]
precision, recall, thresholds = precision_recall_curve(y_test, y_proba)
f1_scores = 2 * precision * recall / (precision + recall + 1e-8)
best_threshold = thresholds[np.argmax(f1_scores)]
print(f"Soglia ottimale trovata: {best_threshold:.3f}")
