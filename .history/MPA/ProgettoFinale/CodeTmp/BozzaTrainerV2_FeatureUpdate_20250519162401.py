...
def extract_features(row):
    ...
    if isinstance(gold_diff, list) and len(gold_diff) > 0:
        ...
        try:
            features["avg_gold_diff_10min"] = np.mean(first_10) # Calcolo la media della gold diff nei primi 10 minuti
            features["gold_diff_min10"] = gold_diff[9] if len(gold_diff) >= 10 else gold_diff[-1] # Ultimo valore della gold diff (minuto 10)
            # Momentum feature: variazione della gold diff
    ...
    b_kills_10min = count_events_in_window(row.get("bKills", []), 10)
    r_kills_10min = count_events_in_window(row.get("rKills", []), 10)
    features["b_kills_10min"] = b_kills_10min
    features["r_kills_10min"] = r_kills_10min
    features["kill_diff_10min"] = b_kills_10min - r_kills_10min

    # Draghi primi 10 minuti
    b_dragons_10min = count_events_in_window(row.get("bDragons", []), 10)
    r_dragons_10min = count_events_in_window(row.get("rDragons", []), 10)
    features["b_dragons_10min"] = b_dragons_10min
    features["r_dragons_10min"] = r_dragons_10min
    features["dragon_diff_10min"] = b_dragons_10min - r_dragons_10min
    ...
    # Eventi dopo il 5° minuto
    features["b_kills_5to10min"] = count_events_in_window(row.get("bKills", []), 10) - count_events_in_window(row.get("bKills", []), 5)
    features["r_kills_5to10min"] = count_events_in_window(row.get("rKills", []), 10) - count_events_in_window(row.get("rKills", []), 5)
    features["b_dragons_5to10min"] = count_events_in_window(row.get("bDragons", []), 10) - count_events_in_window(row.get("bDragons", []), 5)
    features["r_dragons_5to10min"] = count_events_in_window(row.get("rDragons", []), 10) - count_events_in_window(row.get("rDragons", []), 5)
    features["b_towers_5to10min"] = 0
    b_towers_raw = row.get("bTowers", [])
    if isinstance(b_towers_raw, str):
        try:
            b_towers_raw = ast.literal_eval(b_towers_raw)
        except:
            b_towers_raw = []
    if isinstance(b_towers_raw, list):
        for tower in b_towers_raw:
            try:
                minute = float(tower[0])
                if 5 <= minute < 10:
                    features["b_towers_5to10min"] += 1
            except:
                continue

    features["r_towers_5to10min"] = 0
    r_towers_raw = row.get("rTowers", [])
    if isinstance(r_towers_raw, str):
        try:
            r_towers_raw = ast.literal_eval(r_towers_raw)
        except:
            r_towers_raw = []
    if isinstance(r_towers_raw, list):
        for tower in r_towers_raw:
            try:
                minute = float(tower[0])
                if 5 <= minute < 10:
                    features["r_towers_5to10min"] += 1
            except:
                continue
