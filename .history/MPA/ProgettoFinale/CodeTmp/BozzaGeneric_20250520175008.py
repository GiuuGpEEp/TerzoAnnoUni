...
def parse_tower_list_detailed(tower_raw, prefix):
    stats = {f"{prefix}_{key}": 0 for key in [
        "towers_10min", "top_towers", "mid_towers", "bot_towers",
...
def extract_features(row):
    ...
    if isinstance(gold_diff, list) and gold_diff:
        first_10 = gold_diff[:10]
        features["gold_diff_slope_0_10"] = calc_slope(first_10)
        features["gold_diff_slope_0_5"] = calc_slope(first_10[:5])
        features["gold_diff_slope_5_10"] = calc_slope(first_10[5:10])
        ...
        features["gold_diff_delta_last5min"] = first_10[9] - first_10[4] if len(first_10) >= 10 else 0
    else:
        for key in ["gold_diff_slope_0_10", "gold_diff_slope_0_5", "gold_diff_slope_5_10",
                    "avg_gold_diff_10min", "gold_diff_min10", "gold_diff_variance_10min",
                    "gold_diff_delta_last5min"]:
            features[key] = 0
...
def parse_tower_list_detailed_temp(tower_raw, prefix, max_minute):
    stats = {f"{prefix}_towers_destroyed": 0}
    if isinstance(tower_raw, str):
        try:
            tower_raw = ast.literal_eval(tower_raw)
        except:
        ...
        if isinstance(tower, (list, tuple)) and len(tower) >= 3:
            try:
                ...
                if minute < max_minute:
                    stats[f"{prefix}_towers_destroyed"] += 1
            except:
                continue
    return stats

def extract_features_temporal(row, max_minute=10):
    features = {}
    gold_diff = row.get("goldDiff", [])
    if isinstance(gold_diff, str):
        try: gold_diff = ast.literal_eval(gold_diff)
        except: gold_diff = []
    gold_diff = gold_diff[:max_minute]
    if gold_diff:
        features["gold_diff_slope"] = calc_slope(gold_diff)
        features["avg_gold_diff"] = np.mean(gold_diff)
        features["gold_diff_end"] = gold_diff[-1]
        features["gold_diff_variance"] = np.var(gold_diff)
    else:
        for key in ["gold_diff_slope", "avg_gold_diff", "gold_diff_end", "gold_diff_variance"]:
            features[key] = 0
    def parse_event_list_temp(event_raw, prefix, name):
        count = 0
        if isinstance(event_raw, str):
            try: event_raw = ast.literal_eval(event_raw)
            except: event_raw = []
        if isinstance(event_raw, list):
            count = sum(1 for minute in event_raw if isinstance(minute, (int, float)) or str(minute).replace('.', '', 1).isdigit() and float(minute) < max_minute)
        return {f"{prefix}_{name}_count": count}
    for team in ['b', 'r']:
        for event in ['Kills', 'Dragons']:
            features.update(parse_event_list_temp(row.get(f"{team}{event}"), team, event.lower()))
    features["kills_diff"] = features["b_kills_count"] - features["r_kills_count"]
    features["dragons_diff"] = features["b_dragons_count"] - features["r_dragons_count"]
    b_towers = parse_tower_list_detailed_temp(row.get("bTowers"), "b", max_minute)
    r_towers = parse_tower_list_detailed_temp(row.get("rTowers"), "r", max_minute)
    features.update(b_towers)
    features.update(r_towers)
    features["tower_diff"] = b_towers["b_towers_destroyed"] - r_towers["r_towers_destroyed"]
    return pd.Series(features)

# Valutazione per minuti 5, 10, 15, 20

tempo_results = []

for minute in [5, 10, 15, 20]:
    print(f"\n⏱️ Valutazione modello usando i primi {minute} minuti")
    temp_features = df.apply(lambda row: extract_features_temporal(row, max_minute=minute), axis=1).dropna()
    y_temp = df.loc[temp_features.index, "bResult"]

    X_train, X_test, y_train, y_test = train_test_split(temp_features, y_temp, test_size=0.2, stratify=y_temp, random_state=42)
    ...
    tempo_results.append({"Minuti": minute, "Accuracy": acc, "F1-score": f1})

results_df = pd.DataFrame(tempo_results)
plt.figure(figsize=(8, 5))
sns.lineplot(data=results_df, x="Minuti", y="Accuracy", marker="o", label="Accuracy")
sns.lineplot(data=results_df, x="Minuti", y="F1-score", marker="s", label="F1-score")
plt.title("Performance modello in base ai minuti di gioco")
plt.xlabel("Minuti considerati")
plt.ylabel("Score")
plt.legend()
plt.grid(True)
plt.tight_layout()
